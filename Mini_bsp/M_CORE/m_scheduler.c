...
/**
  ...
  * @version V1.0 2019/4/15 -> �޸���tivaware->utils
	* @History V1.0 Create
  ******************************************************************************
  */
tSchedulerTask *g_psSchedulerTable;
uint32_t g_ui32SchedulerNumTasks = SchedulerNumTasks_MAX;
...
static volatile uint16_t Scheduler_Ids;
// 
...
{
	if(Scheduler_Ids>=SchedulerNumTasks_MAX)
		return 0;
	for(uint8_t i = 0; i < Scheduler_Ids; i++ )
	{
		if(!strcmp(Create_Name,g_psSchedulerTable[i].Scheduler_Name))
			return 0 ;
	}
	g_psSchedulerTable[Scheduler_Ids].Scheduler_Name = Create_Name;
	g_psSchedulerTable[Scheduler_Ids].ui32FrequencyTicks = Scheduler_Frenquency;
	g_psSchedulerTable[Scheduler_Ids].bActive = Enable;
	g_psSchedulerTable[Scheduler_Ids].pfnFunction = Func_Callback;
	g_psSchedulerTable[Scheduler_Ids].pvParam = UserParameters;
	Scheduler_Ids++;
	return 1;
}
...
SchedulerInit(uint32_t ui32TicksPerSecond)
{
	...
	SysTick_Config(SystemCoreClock / ui32TicksPerSecond);//���ݲ�ͬ��MCU���и���
    //
